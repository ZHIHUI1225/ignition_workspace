...
class PlanningConfig:
    ...
    def load_config(self):
        ...
        mpc_config = config.get('mpc_controller', {})
        self.mpc_controller = mpc_config
        # Expose individual MPC controller parameters for convenience
        self.mpc_max_linear_velocity = mpc_config.get('max_linear_velocity', 0.05)
        self.mpc_min_linear_velocity = mpc_config.get('min_linear_velocity', 0.0)
        self.mpc_max_angular_velocity = mpc_config.get('max_angular_velocity', 0.4)
        self.mpc_min_angular_velocity = mpc_config.get('min_angular_velocity', -0.4)
        self.mpc_prediction_horizon = mpc_config.get('prediction_horizon', 5)
        self.mpc_position_weight = mpc_config.get('position_weight', 200.0)
        self.mpc_orientation_weight = mpc_config.get('orientation_weight', 3.0)
        self.mpc_velocity_weight = mpc_config.get('velocity_weight', 0.2)
        self.mpc_omega_weight = mpc_config.get('omega_weight', 0.2)
        self.mpc_control_linear_weight = mpc_config.get('control_linear_weight', 0.05)
        self.mpc_control_angular_weight = mpc_config.get('control_angular_weight', 0.3)
        # PI controller fallback
        pi_config = mpc_config.get('pi_controller', {})
        self.pi_kp_position = pi_config.get('kp_position', 0.3)
        self.pi_ki_position = pi_config.get('ki_position', 0.01)
        self.pi_kp_angle = pi_config.get('kp_angle', 0.25)
        self.pi_ki_angle = pi_config.get('ki_angle', 0.005)
