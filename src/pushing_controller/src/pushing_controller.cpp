...
PushingController::PushingController() : rclcpp::Node("pushing_controller")
{
  ...
  obstacles_publisher_ = create_publisher<visualization_msgs::msg::MarkerArray>("pushing/obstacles", 1);
  // services
...
void PushingController::declareRosParameters()
{
  ...
  declare_parameter<double>("robot.x_gain", 10.0);
  declare_parameter<double>("robot.y_gain", 20.0);
  declare_parameter<double>("robot.theta_gain", 2.0);
  declare_parameter<double>("robot.v_gain", 0.05);
  declare_parameter<double>("robot.omega_gain", 0.05);
  ...
  declare_parameter<double>("robot.lin_acc_rate_gain", 1.0);
  declare_parameter<double>("robot.ang_acc_rate_gain", 1.0);
  // pushing mpc
  declare_parameter<double>("pushing.x_gain", 0.05);
  declare_parameter<double>("pushing.y_gain", 0.05);
  declare_parameter<double>("pushing.theta_gain", 0.05);
  declare_parameter<double>("pushing.lin_vel_gain", 1.0);
  declare_parameter<double>("pushing.ang_vel_gain", 1.0);
  declare_parameter<double>("pushing.P_mult", 50.0);
  declare_parameter<double>("pushing.lin_vel_rate_gain", 0.05);
  declare_parameter<double>("pushing.ang_vel_rate_gain", 0.05);
}
...
void PushingController::publishReference()
{
  ...
  for (auto sample : pushing_trajectory_.action.trajectory) {
    geometry_msgs::msg::PoseStamped p;
    p.header = sample.header;
    p.pose = sample.pose;
...
{
  // generate obstacles from map
  obstacles_.clear();
  double map_x = req->map.info.origin.position.x;
  double map_y = req->map.info.origin.position.y;
  double map_yaw = tf2::getYaw(req->map.info.origin.orientation);
  double cos_map_yaw = cos(map_yaw);
  double sin_map_yaw = sin(map_yaw);
  auto map_width = req->map.info.width;
  auto map_height = req->map.info.height;
  auto map_resolution = req->map.info.resolution;
  for (size_t i = 0; i < map_height; i++) {
    for (size_t j = 0; j < map_width; j++) {
      if (req->map.data[i * map_width + j] > 50) {
        geometry_msgs::msg::Pose obstacle;
        double cell_x = map_resolution * j;
        double cell_y = map_resolution * i;
        obstacle.position.x = map_x + cos_map_yaw * cell_x - sin_map_yaw * cell_y;
        obstacle.position.y = map_y + sin_map_yaw * cell_x + cos_map_yaw * cell_y;
...
void PushingController::updateObstaclesMPC()
{
  double px = robot_state[0];
  double py = robot_state[1];
  try {
    std::sort(obstacles_.begin(), obstacles_.end(), [px, py](geometry_msgs::msg::Pose a, geometry_msgs::msg::Pose b) {
      return sqrt(pow(a.position.x - px, 2) + pow(a.position.y - py, 2)) < sqrt(pow(b.position.x - px, 2) + pow(b.position.y - py, 2));
    });
  } catch (const std::exception& e) {
    std::cerr << e.what() << '\n';
  }

  robot_param.q1[0] = obstacles_.at(0).position.x;
  robot_param.q1[1] = obstacles_.at(0).position.y;
  robot_param.q2[0] = obstacles_.at(1).position.x;
  robot_param.q2[1] = obstacles_.at(1).position.y;
  robot_param.q3[0] = obstacles_.at(2).position.x;
  robot_param.q3[1] = obstacles_.at(2).position.y;
  robot_param.q4[0] = obstacles_.at(3).position.x;
  robot_param.q4[1] = obstacles_.at(3).position.y;
  robot_param.q5[0] = obstacles_.at(4).position.x;
  robot_param.q5[1] = obstacles_.at(4).position.y;
  pushing_param.q1[0] = obstacles_.at(0).position.x;
  pushing_param.q1[1] = obstacles_.at(0).position.y;
  pushing_param.q2[0] = obstacles_.at(1).position.x;
  pushing_param.q2[1] = obstacles_.at(1).position.y;
  pushing_param.q3[0] = obstacles_.at(2).position.x;
  pushing_param.q3[1] = obstacles_.at(2).position.y;
  pushing_param.q4[0] = obstacles_.at(3).position.x;
  pushing_param.q4[1] = obstacles_.at(3).position.y;
  pushing_param.q5[0] = obstacles_.at(4).position.x;
  pushing_param.q5[1] = obstacles_.at(4).position.y;
}
void PushingController::publishObstacles()
{
  ...
  marker.scale.x = 2 * robot_param.safe_d;
  marker.scale.y = 2 * robot_param.safe_d;
  marker.scale.z = 0.2;
  marker.color.a = 1.0;
  marker.color.r = 1.0;
  ...
  marker.id = 0;
  marker.pose.position.x = robot_param.q1[0];
  marker.pose.position.y = robot_param.q1[1];
  marker.pose.position.z = 0.1;
  msg.markers.push_back(marker);

  marker.id = 1;
  marker.pose.position.x = robot_param.q2[0];
  marker.pose.position.y = robot_param.q2[1];
  marker.pose.position.z = 0.1;
  msg.markers.push_back(marker);

  marker.id = 2;
  marker.pose.position.x = robot_param.q3[0];
  marker.pose.position.y = robot_param.q3[1];
  marker.pose.position.z = 0.1;
  msg.markers.push_back(marker);

  marker.id = 3;
  marker.pose.position.x = robot_param.q4[0];
  marker.pose.position.y = robot_param.q4[1];
  marker.pose.position.z = 0.1;
  msg.markers.push_back(marker);

  marker.id = 4;
  marker.pose.position.x = robot_param.q5[0];
  marker.pose.position.y = robot_param.q5[1];
  marker.pose.position.z = 0.1;
  msg.markers.push_back(marker);
