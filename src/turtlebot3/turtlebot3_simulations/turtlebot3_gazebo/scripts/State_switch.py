...
class ProximityChecker(Node):
    def __init__(self): # Removed namespace argument
        ...
        self.get_logger().info(f"State_switch node initialized for namespace: {namespace}") # Log namespace
        ...
        with self.lock:
            ...
            if   self.namespace_number!=0 and self.last_flag is True and dis_last_parcel > 0.25 and self.pushing_flag is False:
                if dis_robot_parcel > 0.14: # the last robot has pushed the parcel to the relay point
                    ...
                    self.current_state = np.array([self.robot.pose.position.x, self.robot.pose.position.y, self.quaternion_to_yaw(self.robot.pose.orientation)])
...
def main(args=None):
    ...
    # Remove auxiliary node creation
