...
from launch.actions import DeclareLaunchArgument, RegisterEventHandler, IncludeLaunchDescription, SetEnvironmentVariable, OpaqueFunction, TimerAction, ExecuteProcess
...
def generate_launch_description():
    ...
    # Check if Waypoints.json file exists before proceeding with robot spawning
    try:
        ...
        # Spawn parcel for each robot (initial)
        ...
        print(f"Warning: {json_file_path} not found. Spawning a single robot instead.")
        
        # Spawn a single turtlebot if no waypoints file
        spawn_turtlebot3_burger = Node(
            package="ros_gz_sim",
            executable="create",
            arguments=[
        ...
        print(f"Warning: {json_file_path} is not a valid JSON file. Spawning a single robot instead.")
        
        # Spawn a single turtlebot if waypoints file is invalid
        spawn_turtlebot3_burger = Node(
            package="ros_gz_sim",
            executable="create",
            arguments=[
    ...
    bridge_node = Node(
        ...
        parameters=[{
            ...
            'frequency': 20.0  # Set parcel pose publishing frequency to 20 Hz
        }]
    )
    ...
    # Uncomment the following line to enable RViz
